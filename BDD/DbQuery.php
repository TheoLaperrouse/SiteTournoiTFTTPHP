<?php
namespace BDD;
class DbQuery extends \BDD\SGBD
{
    /**
     ...
     * @return DbQuery
     */
    public function from($table, $prefix = _DB_PREFIX_, $alias = null)
    {
        if (!empty($table)) {
			if (!is_array($this->query['from'])) $this->query['from'] = array();
            $this->query['from'][] = '`'.$prefix.$table.'`'.($alias ? ' '.$alias : '');
        }
     ...
     * @return DbQuery
     */
    public function leftJoin($table, $prefix = _DB_PREFIX_, $alias = null, $on = null)
    {
        return $this->join('LEFT JOIN `'.$prefix.\Tools::bqSQL($table).'`'.($alias ? ' `'.\Tools::pSQL($alias).'`' : '').($on ? ' ON '.$on : ''));
    }
     ...
     * @return DbQuery
     */
    public function innerJoin($table, $prefix = _DB_PREFIX_, $alias = null, $on = null)
    {
        return $this->join('INNER JOIN `'.$prefix.\Tools::bqSQL($table).'`'.($alias ? ' '.\Tools::pSQL($alias) : '').($on ? ' ON '.$on : ''));
    }
     ...
     * @return DbQuery
     */
    public function leftOuterJoin($table, $prefix = _DB_PREFIX_, $alias = null, $on = null)
    {
        return $this->join('LEFT OUTER JOIN `'.$prefix.\Tools::bqSQL($table).'`'.($alias ? ' '.\Tools::pSQL($alias) : '').($on ? ' ON '.$on : ''));
    }
     ...
     * @return DbQuery
     */
    public function naturalJoin($table, $prefix = _DB_PREFIX_, $alias = null)
    {
        return $this->join('NATURAL JOIN `'.$prefix.\Tools::bqSQL($table).'`'.($alias ? ' '.\Tools::pSQL($alias) : ''));
    }
     ...
     * @return DbQuery
     */
    public function rightJoin($table, $prefix = _DB_PREFIX_, $alias = null, $on = null)
    {
        return $this->join('RIGHT JOIN `'.$prefix.\Tools::bqSQL($table).'`'.($alias ? ' `'.\Tools::pSQL($alias).'`' : '').($on ? ' ON '.$on : ''));
    }
